...
nvch_dir="${NV_CODEC_HEADERS:-$(cygpath -m "${nvch_default}")}"
vulkan_sdk="${VULKAN_SDK:-C:/VulkanSDK/1.4.357.0}"
...
if [[ -f "${nvch_dir}/include/ffnvcodec/dynlink_cuda.h" ]]; then
  ...
  hwaccel_includes+=" /I$(cygpath -m "${nvch_dir}/include")"
...
if [[ -f "${vulkan_include}/vulkan/vulkan.h" ]]; then
  ...
  hwaccel_includes+=" /I$(cygpath -m "${vulkan_include}")"
